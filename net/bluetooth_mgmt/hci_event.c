...
/* SS_BLUETOOTH(insick.jung) 2012.12.27 */
/* PAN connection count issue in Qcom BT */
	...
	if ((mask & HCI_LM_ACCEPT) &&
		...
		if (ev->link_type == ACL_LINK || !lmp_esco_capable(hdev)) {
			...
			if (lmp_rswitch_capable(hdev)
				&& (is_role_switch_required(hdev) || (mask & HCI_LM_MASTER))) {
				BT_DBG("Connection as slave role exists");
...
static inline void hci_encrypt_change_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (conn) {
		...
		* to disable encryption +   * for short duration in this case.
		*/
		if ((ev->encrypt == 0) && (ev->status == 0) &&
		    ((conn->features[5] & LMP_PAUSE_ENC) == 0)) {
		    mod_timer(&conn->encrypt_disabled_timer,
		    jiffies + msecs_to_jiffies(500));
		    BT_INFO("enc pause timer, enc_pend_flag set");
		} else {
			del_timer(&conn->encrypt_disabled_timer);
