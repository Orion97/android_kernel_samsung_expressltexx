...
static struct msm_gpiomux_config msm8930_mhl_configs_rev04[] __initdata = {
	{
		.gpio = GPIO_MHL_SEL,
		.settings = {
			[GPIOMUX_ACTIVE]	= &mhl_active_1_cfg,
...
static struct msm_gpiomux_config msm8930_mhl_configs_rev06[] __initdata = {
	{
		.gpio = GPIO_MHL_SEL,
		.settings = {
			[GPIOMUX_ACTIVE]	= &mhl_active_1_cfg,
...
int __init msm8930_init_gpiomux(void)
{
	...
	if (system_rev < BOARD_REV04)
		...
		msm_gpiomux_install(msm8930_mhl_configs_rev04,
				ARRAY_SIZE(msm8930_mhl_configs_rev04));
	else
		msm_gpiomux_install(msm8930_mhl_configs_rev06,
				ARRAY_SIZE(msm8930_mhl_configs_rev06));
#endif

	msm_gpiomux_install(msm8960_mdp_vsync_configs,
			ARRAY_SIZE(msm8960_mdp_vsync_configs));
	gpio_keys_config_mux[2].gpio = gpio_rev(HOME_KEY);
	if (system_rev >= BOARD_REV01)
		gpio_keys_config_mux[2].settings[GPIOMUX_ACTIVE] =
				&gpio_keys_pullup_config;
	if (system_rev >= BOARD_REV02) {
		gpio_keys_config_mux[1].settings[GPIOMUX_ACTIVE] =
				&gpio_keys_pullup_config;
		gpio_keys_config_mux[0].settings[GPIOMUX_ACTIVE] =
				&gpio_keys_pullup_config;
	}
