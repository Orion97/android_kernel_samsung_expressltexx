...
static bool sec_bat_gpio_init(void)
{
	...
	gpio_tlmm_config(GPIO_CFG(GPIO_CHARGER_CHG_nSTAT, 0, GPIO_CFG_INPUT,
		 GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
...
static bool sec_chg_gpio_init(void)
{
	...
	gpio_tlmm_config(GPIO_CFG(GPIO_CHARGER_CHG_EN,  0, GPIO_CFG_OUTPUT,
		 GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
...
static bool sec_bat_check_callback(void)
{
	...
	for (i = 0; i < 5; i++) {
		msleep(500);
		present = !gpio_get_value_cansleep(GPIO_BATT_INT);
...
/*
Function : sec_bat_check_result_callback
Description :
*/
...
static sec_battery_platform_data_t sec_battery_pdata = {
	...
	/* Charger */
	.chg_gpio_en = GPIO_CHARGER_CHG_EN,
	.chg_polarity_en = 0,
	.chg_gpio_status = 0,
