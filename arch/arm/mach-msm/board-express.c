...
int gpio_rev(unsigned int index)
{
	...
	/* Return last HOME KEY value for the next revision device */
...
#define MSM_ION_QSECOM_SIZE	0x1400000 /* (20MB) request from sangjun */
...
static void vps_dock_sel(bool onoff)
{
	if (system_rev < BOARD_REV04) {
		gpio_direction_output(GPIO_MHL_SEL, onoff ? 1 : 0);
		gpio_direction_output(GPIO_VPS_SEL, 0);
		pr_info("vps_sel :%d mhl_sel :%d in fsa9485\n",
					gpio_get_value_cansleep(GPIO_VPS_SEL),
...
static void msm8930_mhl_gpio_init(void)
{
	...
	if (system_rev < BOARD_REV04) {
		ret = gpio_request(GPIO_VPS_SEL, "vps_sel");
		if (ret < 0) {
			pr_err("vps_sel gpio_request is failed\n");
			return;
		}
	}
...
static void mhl_gpio_config(void)
{
	...
	if (system_rev < BOARD_REV04)
		gpio_tlmm_config(GPIO_CFG(GPIO_VPS_SEL, 0, GPIO_CFG_OUTPUT,
					GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), 1);
	if (system_rev < BOARD_REV06)
...
{
	if (system_rev < BOARD_REV04) {
		gpio_direction_output(GPIO_MHL_SEL, 0);
		gpio_direction_output(GPIO_VPS_SEL, onoff ? 1 : 0);
		pr_info("vps_sel :%d mhl_sel :%d in fsa9485\n",
					gpio_get_value_cansleep(GPIO_VPS_SEL),
					gpio_get_value_cansleep(GPIO_MHL_SEL));
	} else {
...
{
	/*KEY REV*/
	gpio_keys_button[2].gpio = gpio_rev(HOME_KEY);
}
...
static void __init msm8930_pm8917_pdata_fixup(void)
{
	...
	msm_device_saw_core0.dev.platform_data
		= &msm8930_pm8917_saw_regulator_core0_pdata;
	msm_device_saw_core1.dev.platform_data
		= &msm8930_pm8917_saw_regulator_core1_pdata;
...
static void __init msm8930_express_init(void)
{
#ifdef CONFIG_SEC_DEBUG
	sec_debug_init();
#endif
...
MACHINE_START(EXPRESS , "SAMSUNG EXPRESS")
	...
	.timer = &msm_timer,
	.init_machine = msm8930_express_init,
