...
void msmsdcc_dump_sdcc_state(struct msmsdcc_host *host);
...
static inline unsigned short msmsdcc_get_nr_sg(struct msmsdcc_host *host)
{
	...
	if (is_sps_mode(host)) {
		/* to avoid the allocation error
		   when the fragmention is occured */
...
EXPORT_SYMBOL(msmsdcc_dump_sdcc_state);
...
#define MSM_MMC_DEFAULT_CPUDMA_LATENCY 200 /* usecs */
	...
	/* mmc->caps2 |= MMC_CAP2_SANITIZE;   */ /* Disable Sanitize for TLC */
	/* mmc->caps2 |= MMC_CAP2_INIT_BKOPS; */ /* Disable BKOPS for TLC */
