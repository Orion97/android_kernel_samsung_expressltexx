...
static int alloc_mem(struct fastrpc_buf *buf)
{
	...
	if (err)
		goto bail;
	VERIFY(0 != (sg = ion_sg_table(clnt, buf->handle)));
	if (err)
		goto bail;
	VERIFY(1 == sg->nents);
	if (err)
			...
			int *nbufs)
{
	...
	for (i = 0; i < inbufs + outbufs; ++i) {
		...
		if (i < inbufs) {
			if (!kernel) {
				VERIFY(0 == copy_from_user(args, pra[i].buf.pv,
							pra[i].buf.len));
				if (err)
					goto bail;
			} else {
			...
			remote_arg_t *rpra, remote_arg_t *upra)
{
	...
	for (i = inbufs; i < inbufs + outbufs; ++i) {
		if (rpra[i].buf.pv != pra[i].buf.pv) {
			VERIFY(0 == copy_to_user(pra[i].buf.pv,
...
static int __init fastrpc_device_init(void)
{
	...
	if (IS_ERR(me->class)) {
		pr_info("'failed to create class for %s c %d 0'\n",
			DEVICE_NAME, MAJOR(me->dev_no));
		err = -1;
		goto bail;
	}
	me->dev = device_create(me->class,
		NULL, MKDEV(MAJOR(me->dev_no), 0), NULL, DEVICE_NAME);
	if (IS_ERR(me->dev)) {
		pr_info("'failed to create dev for %s c %d 0'\n",
			DEVICE_NAME, MAJOR(me->dev_no));
		err = -1;
		goto bail;
	}
	pr_info("'created %s c %d 0'\n", DEVICE_NAME, MAJOR(me->dev_no));
