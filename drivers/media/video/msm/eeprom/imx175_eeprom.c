...
/*	{0x002000, &imx175_lsccalib_data[0], sizeof(FRom_lsc), 0},*//*Test*/
...
/*Start : shchang@qualcomm.com : 1122*/
	if (((uint16_t)ftov_BEn(af->position)) == 0x00) {
		if (((uint16_t)ftov_LEn(af->position)) == 0x00) {
			imx175_calib_supp_info.af.is_supported = FALSE;
			pr_err("[shchang : %s] No data!!\n", __func__);
		} else {
			imx175_af_data.start_dac
				= (uint16_t)ftov_LEn(af->position);
			pr_err("[shchang : %s] Little Endian!!\n", __func__);
		}
	} else {
		imx175_af_data.start_dac = (uint16_t)ftov_BEn(af->position);
...
static void imx175_format_wbdata(void)
{
	...
	/*pr_err("%s: [Total]wb->rg = %d, wb->bg = %d,
	gld_wb->rg = %d, gld_wb->bg = %d\n", __func__,
	wb->rg, wb->bg, gld_wb->rg, gld_wb->bg);*/

	/*Start : shchang@qualcomm.com : 1122*/
	/*pr_err("%s: [Big]wb->rg = %d, wb->bg = %d,
	gld_wb->rg = %d, gld_wb->bg = %d\n", __func__,
	(uint16_t)ftov_BEn(wb->rg), (uint16_t)ftov_BEn(wb->bg),
	(uint16_t)ftov_BEn(gld_wb->rg), (uint16_t)ftov_BEn(gld_wb->bg));

	pr_err("%s: [Little]wb->rg = %d, wb->bg = %d, gld_wb->rg = %d,
	gld_wb->bg = %d\n", __func__,
	(uint16_t)ftov_LEn(wb->rg), (uint16_t)ftov_LEn(wb->bg),
	(uint16_t)ftov_LEn(gld_wb->rg), (uint16_t)ftov_LEn(gld_wb->bg));*/

	if (((uint16_t)ftov_BEn(wb->rg)) == 0x00 &&
		((uint16_t)ftov_BEn(wb->bg)) == 0x00 &&
		((uint16_t)ftov_BEn(gld_wb->rg)) == 0x00 &&
		((uint16_t)ftov_BEn(gld_wb->bg) == 0x00)) {
		if (((uint16_t)ftov_LEn(wb->rg)) == 0x00 &&
			((uint16_t)ftov_LEn(wb->bg)) == 0x00 &&
			((uint16_t)ftov_LEn(gld_wb->rg)) == 0x00 &&
			((uint16_t)ftov_LEn(gld_wb->bg) == 0x00)) {
			...
			imx175_gld_wb_data.r_over_g =
				(uint16_t)ftov_LEn(gld_wb->rg);
			imx175_gld_wb_data.b_over_g =
				(uint16_t)ftov_LEn(gld_wb->bg);
			imx175_gld_wb_data.gr_over_gb = 1;
			/*(uint16_t)gld_wb->gr_gb;*/
			pr_err("[shchang : %s] Little Endian!!\n", __func__);
		}
	} else {
		...
		imx175_gld_wb_data.r_over_g = (uint16_t)ftov_BEn(gld_wb->rg);
		imx175_gld_wb_data.b_over_g = (uint16_t)ftov_BEn(gld_wb->bg);
...
uint8_t imx175_check_module_info(void)
{
	...
	pr_err("[shchang:CAL mod_info] %x:%x:%x:%x\n",
...
void imx175_format_calibrationdata(void)
{
	...
	pr_err("%s: is_all_supported= %d", __func__, is_all_supported);
	if (is_all_supported) {
		/* patch#183359 : e*/
