...
static int msm_open(struct file *f)
{
	...
	if (pcam->use_count == 1) {
		...
		pmctl = msm_cam_server_get_mctl(pcam->mctl_handle);
		pr_err("SURESH: msm_mctl_dev_close: vb2_queue_release-2\n");
		if (!pmctl) {
...
void msm_release_ion_client(struct kref *ref)
{
	...
	pr_err("SURESH: %s Calling ion_client_destroy :%x \n",
		__func__, (unsigned int) mctl);
...
static int msm_close(struct file *f)
{
	...
	if (pcam_inst->vbqueue_initialized) {
		pr_err("SURESH: msm_close: vb2_queue_release\n");
		vb2_queue_release(&pcam_inst->vid_bufq);
		pr_err("SURESH: msm_close: vb2_queue_release -2\n");
	}
	...
	kfree(pcam_inst);
	f->private_data = NULL;
	printk(KERN_ERR "msm_close :  use_count %d\n", pcam->use_count);
		...
		bool valid_prio, int cmd, void *arg)
{
	...
	int i;
	/*struct ioctl_native_cmd cam_info;*/ /*v4l2_io_ctl*/
	...
	case MSM_CAM_V4L2_IOCTL_GET_CAMERA_INFO:
		...
		temp_cam_info.num_cameras =
			g_server_dev.camera_info.num_cameras;*/ // LA1.7 camera temp
	...
	case MSM_CAM_V4L2_IOCTL_GET_CONFIG_INFO:
		...
		temp_config_info.num_config_nodes =
			g_server_dev.config_info.num_config_nodes;*/ // LA1.7 camera temp
		if (copy_to_user((void __user *)ioctl_ptr->ioctl_ptr,
							  &temp_config_info,
				sizeof(struct msm_cam_config_dev_info))) {
			rc = -EINVAL;
	...
	case MSM_CAM_V4L2_IOCTL_GET_MCTL_INFO:
		...
		temp_mctl_info.num_mctl_nodes =
			g_server_dev.mctl_node_info.num_mctl_nodes;*/ // LA1.7 camera temp
		if (copy_to_user((void __user *)ioctl_ptr->ioctl_ptr,
							  &temp_mctl_info,
				sizeof(struct msm_mctl_node_info))) {
			rc = -EINVAL;
	...
	case MSM_CAM_V4L2_IOCTL_CTRL_CMD_DONE:
		...
		/*rc = msm_ctrl_cmd_done(arg);*/ // LA1.7 camera temp
		break;
	...
	case MSM_CAM_V4L2_IOCTL_GET_EVENT_PAYLOAD: {
		...
		queue = &g_server_dev.server_queue
			[u_isp_event.isp_data.ctrl.queue_idx].eventData_q;*/ // LA1.7 camera temp
		event_cmd = msm_dequeue(queue, list_eventdata);
		if (!event_cmd) {
			pr_err("%s: No event payload\n", __func__);
			rc = -EINVAL;
			/*mutex_unlock(&g_server_dev.server_queue_lock);*/ // LA1.7 camera temp
			return rc;
		}
	...
	case MSM_CAM_IOCTL_SEND_EVENT:
		/*rc = msm_server_send_v4l2_evt(arg);*/ // LA1.7 camera temp
		break;

#if defined(CONFIG_MACH_EXPRESS) /*v4l2_ioctl*/
...
static int msm_open_server(struct file *fp)
{
	...
	mutex_unlock(&g_server_dev.server_lock);*/ // LA1.7 camera temp
	return rc;
}
...
static int msm_close_server(struct file *fp)
{
	struct v4l2_event_subscription sub;
	D("%s\n", __func__);
	/*mutex_lock(&g_server_dev.server_lock);*/ // LA1.7 camera temp
	/*if (g_server_dev.use_count > 0)
		g_server_dev.use_count--;
	mutex_unlock(&g_server_dev.server_lock);*/ // LA1.7 camera temp
#if 0 // LA1.7 camera temp
	if (g_server_dev.use_count == 0) {
		mutex_lock(&g_server_dev.server_lock);
