...
static int32_t db8131m_sensor_setting(int update_type, int rt)
{
	...
	case UPDATE_PERIODIC:
		...
		if (rt == RES_PREVIEW || rt == RES_CAPTURE) {
			...
			msm_sensor_enable_debugfs(db8131m_ctrl->s_ctrl);
			/*v4l2_subdev_notify(&db8131m_ctrl->s_ctrl->
					sensor_v4l2_subdev,
			...
			if (db8131m_ctrl->s_ctrl->curr_csi_params !=
				db8131m_ctrl->s_ctrl->csi_params[0]) {
				CAM_DEBUG(" start AP MIPI setting");
				db8131m_ctrl->s_ctrl->curr_csi_params =
					db8131m_ctrl->s_ctrl->csi_params[0];
				CAM_DEBUG(" CSI settle_cnt"	" 0x%x",
					db8131m_ctrl->s_ctrl->
					curr_csi_params->
					csiphy_params.settle_cnt);
				if (db8131m_ctrl->s_ctrl->sensordata->
					sensor_platform_info->
					csi_lane_params != NULL) {
					CAM_DEBUG(" lane_assign =" " 0x%x",
						db8131m_ctrl->s_ctrl->
						sensordata->
						sensor_platform_info->
						csi_lane_params->
...
static int db8131m_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	int rc = 0;
...
static int db8131m_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	gpio_set_value(data->sensor_platform_info->vt_sensor_stby, 0);
	temp = gpio_get_value(data->sensor_platform_info->vt_sensor_stby);
	if (temp == 1)
		cam_info("check VT standby : %d", temp);

	gpio_set_value(data->sensor_platform_info->vt_sensor_reset, 0);
	temp = gpio_get_value(data->sensor_platform_info->vt_sensor_reset);
	if (temp == 1)
		cam_info("check VT reset : %d", temp);
	udelay(1000);
	...
	udelay(1000);
	/*standy VT */
	gpio_set_value(data->sensor_platform_info->vt_sensor_stby, 1);
	temp = gpio_get_value(data->sensor_platform_info->vt_sensor_stby);
	if (temp == 1)
		cam_info("check VT standby : %d", temp);
	usleep(5000);
	...
	/*reset VT */
	gpio_set_value(data->sensor_platform_info->vt_sensor_reset, 1);
	temp = gpio_get_value(data->sensor_platform_info->vt_sensor_reset);
	if (temp == 1)
		cam_info("check VT reset : %d", temp);
	usleep(5000); /* > 70000cycle */
...
*/
	if (rc < 0) {
		cam_err("Error in Front Camera Sensor Validation Test");
		return rc;
	}
	config_csi2 = 0;
...
static int db8131m_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	gpio_set_value(data->sensor_platform_info->sensor_reset, 0);
	temp = gpio_get_value(data->sensor_platform_info->sensor_reset);
	CAM_DEBUG("CAM_5M_RST : %d", temp);

	gpio_set_value(data->sensor_platform_info->sensor_stby, 0);
...
void sensor_native_control_front(void __user *arg)
{
	...
	default:
		cam_info("There is nothing to handle");
		break;
	}
	...
	const struct i2c_device_id *id)
{
	...
	if (!db8131m_ctrl) {
		cam_err("db8131m_ctrl alloc failed!\n");
		rc = -ENOMEM;
		goto probe_failure;
	}
...
*/
	cam_err("probe succeeded!");
	return 0;

probe_failure:
	cam_err("probe failed!");
	kfree(db8131m_exif);
	return rc;
}

static struct msm_camera_csi_params db8131m_csic_params = {
...
static struct msm_camera_csi_params *db8131m_csic_params_array[] = {
	&db8131m_csic_params,
	&db8131m_csic_params,
};

static struct msm_camera_csid_vc_cfg db8131m_cid_cfg[] = {
	{0, CSI_YUV422_8, CSI_DECODE_8BIT},
	{1, CSI_EMBED_DATA, CSI_DECODE_8BIT},
	{2, CSI_RESERVED_DATA_0, CSI_DECODE_8BIT},
};

static struct msm_camera_csi2_params db8131m_csi_params = {
	.csid_params = {
		.lane_cnt = 1,
		.lut_params = {
			.num_cid = ARRAY_SIZE(db8131m_cid_cfg),
			.vc_cfg = db8131m_cid_cfg,
		},
	},
	.csiphy_params = {
		.lane_cnt = 1,
...
static struct msm_camera_csi2_params *db8131m_csi_params_array[] = {
	&db8131m_csi_params,
	&db8131m_csi_params,
};

/* this temp code */
static struct msm_sensor_id_info_t db8131m_id_info = {
...
static struct msm_sensor_fn_t db8131m_func_tbl = {
	...
	.sensor_match_id = db8131m_sensor_match_id,
};
...
static struct msm_sensor_ctrl_t db8131m_s_ctrl = {
	...
	.sensor_i2c_addr = 0xC1,
	.sensor_id_info = &db8131m_id_info,
	.cam_mode = MSM_SENSOR_MODE_INVALID,
	.csic_params = &db8131m_csic_params_array[0],
	.csi_params = &db8131m_csi_params_array[0],
