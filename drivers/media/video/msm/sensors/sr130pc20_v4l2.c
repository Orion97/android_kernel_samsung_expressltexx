...
void sr130pc20_set_frame_rate(int32_t fps)
{
	CAM_DEBUG(" %d", fps);
	 if (fps == 15) {
		if(sr130pc20_ctrl->mirror_mode == 1) {
			sr130pc20_WRT_LIST(sr130pc20_15fps_mirror);
		} else {
			sr130pc20_WRT_LIST(sr130pc20_15fps_no_mirror);
		}
	}

	sr130pc20_ctrl->settings.fps = fps;
}
...
*/
	...
	case EXT_CAM_SET_FPS:
		sr130pc20_set_frame_rate(ctrl_info.value_1);
		break;
...
void sr130pc20_sensor_start_stream(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (sr130pc20_ctrl->cam_mode == MOVIE_MODE) {
		...
		if (sr130pc20_ctrl->op_mode == CAMERA_MODE_INIT ||
			...
			/*if (sr130pc20_ctrl->settings.fps == 15) {
				CAM_DEBUG("MMS recording");
				sr130pc20_WRT_LIST(sr130pc20_Wifi_15fps);
				}*/
