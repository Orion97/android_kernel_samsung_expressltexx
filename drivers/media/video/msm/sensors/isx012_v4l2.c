...
static int isx012_i2c_write_multi_temp(unsigned short addr,
	unsigned int w_data, unsigned int w_len)
{
	unsigned char buf[w_len+2];
				 ...
				 char *name)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (temp_i2c_address == 1)
			err = isx012_i2c_write_multi_temp(regs[i].subaddr,
...
static int isx012_mode_transtion_OM_temp(void)
{
	int count = 0;
	int status = 0;
	int om_state = 0;
	...
	for (count = 0; count < 100; count++) {
		ret = isx012_i2c_read_temp(0x000E, (unsigned short *)&status);
		if (ret < 0) {
			if (retry_cnt > I2C_RETRY_CNT) {
				cam_err("isx012_i2c_read failed!!\n");
				return -EIO;
...
static int isx012_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	err = isx012_mode_transtion_OM_temp();
	if (err == -EIO) {
		cam_err("[isx012] start1 fail!\n");
		return -EIO;
		}
