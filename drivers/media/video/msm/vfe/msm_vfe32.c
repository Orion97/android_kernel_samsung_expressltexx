...
/*Start : shchang@qualcomm.com : 1101*/
		pr_err("[shchang : BUS1] cpu_is_msm8930aa() = %d\n",
			cpu_is_msm8930aa());
		if (cpu_is_msm8930aa()) {
	...
	struct vfe32_ctrl_type *vfe32_ctrl)
{
	...
	case VFE_CMD_RGB_G_CFG: {
	    ...
	    cmdp -= 1;
		break;
/*Start : shchang@qualcomm.com : 1206*/
...
/*End : shchang@qualcomm.com : 1101*/
	...
	if (axi_ctrl->share_ctrl->stop_immediately) {
		...
		if (axi_ctrl->share_ctrl->stream_error == 1) {
			pr_err("Stream error happened.");
			/*H/W might not be able to give stop ack");*/
		vfe32_send_isp_msg(
		&(axi_ctrl->share_ctrl->vfe32_ctrl->subdev),
		axi_ctrl->share_ctrl->vfe32_ctrl->share_ctrl->vfeFrameId,
