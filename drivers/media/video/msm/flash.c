...
bool is_stunner_rev08()
{
#ifdef CONFIG_MACH_STUNNER
	if (gpio_get_value_cansleep(97) == 1)
		return true;
...
int get_gpio_ENM(struct msm_camera_sensor_flash_external *external)
{
	if ((gpio_get_value_cansleep(97) == 0) &&
		(gpio_get_value_cansleep(93) == 0) &&
		(is_godiva())) {
		cam_err("[GODIVA :: << REV04]\n");
		return gpio_get_value_cansleep(external->led_en);
	} else {
		cam_err("[GODIVA :: >> REV04][or][STUNNER]\n");
		...
		unsigned led_state)
{
	...
	int rc = 0;
	int i = 0;
	int assistive = get_gpio_ENM(external);

	cam_err("[Assistive Light::%d]\n", assistive);
	if (assistive == 1) {
		cam_err("[%s][Assistive Light On][Forced Return]\n", __func__);
		return rc;
	}
	...
	case MSM_CAMERA_LED_HIGH:
		...
		if (is_godiva() || is_stunner_rev08()) {
			cam_err("[GODIVA or STUNNER REV08 :: 16 toggle]\n");
			i = 16;
		} else {
			cam_err("[STUNNER under REV08 :: 10 toggle]\n");
			i = 10;
		}
