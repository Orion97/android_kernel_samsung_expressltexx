...
static void wait_for_xmitr(struct uart_port *port)
{
	...
	if (!(msm_hsl_read(port, regmap[vid][UARTDM_SR]) &
		...
		/*
		 * Rollback the "Check SR[TXEMT] in wait_for_xmitr" QC patch
		 * to solve the serial watchdog bark problem by SS
		 */
		while ((msm_hsl_read(port, regmap[vid][UARTDM_ISR]) &
			UARTDM_ISR_TX_READY_BMSK) != UARTDM_ISR_TX_READY_BMSK) {
			udelay(1);
			touch_nmi_watchdog();
			cpu_relax();
...
static int msm_serial_hsl_suspend(struct device *dev)
{
	...
	if (port) {
		...
		/*  KONA Model does not have fsa9485 chip
