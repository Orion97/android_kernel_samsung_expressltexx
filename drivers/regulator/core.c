...
static int regulator_debug_print_showall(void *data, u64 *va)
{
	return showall_enabled();
}

DEFINE_SIMPLE_ATTRIBUTE(regulator_showall_fops, regulator_debug_print_showall,
			NULL, "%llu\n");
...
static int __init regulator_init(void)
{
	...
	if (!debugfs_create_file("showall", S_IRUGO, debugfs_root, NULL,
				&regulator_showall_fops)) {
		debugfs_remove_recursive(debugfs_root);
		return -ENOMEM;
	}
#endif
