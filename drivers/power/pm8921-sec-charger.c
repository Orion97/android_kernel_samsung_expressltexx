...
static void update_heartbeat(struct work_struct *work)
{
	...
	pr_info("health(%d),stat(%d),chgen(%d),fsm(%d),cable(%d),extchg(%d),uvd(%d)\n",
...
static irqreturn_t wpc_charger_irq(int irq, void *data)
{
	...
	if ((chip->wc_w_state == 0) && (wc_w_state == 1)) {
		chip->uvd_voltage_mv = 3850;
		rc = pm_chg_uvd_threshold_set(chip, chip->uvd_voltage_mv);
		if (rc)
			pr_err("Failed to set UVD threshold %d, rc=%d\n",
					chip->uvd_voltage_mv, rc);
		else
			pr_info("WPC : UVD threshold changed ( %d )\n",
					chip->uvd_voltage_mv);
		...
		chip->uvd_voltage_mv = chip->chg_pdata->uvd_thresh_voltage;
		rc = pm_chg_uvd_threshold_set(chip, chip->uvd_voltage_mv);
		if (rc)
			pr_err("Failed to set UVD threshold %d, rc=%d\n",
					chip->uvd_voltage_mv, rc);
