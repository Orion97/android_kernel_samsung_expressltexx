...
static void calculate_cc_uah(struct pm8921_bms_chip *chip, int cc, int *val)
{
	...
	pr_info("cc = %d. after subtracting 0x%x cc = %lld\n",
	...
	pr_info("cc_uvh = %lld micro_volt_hour\n", cc_uvh);
						...
						int *iavg_ua)
{
	...
	*remaining_charge_uah = calculate_remaining_charge_uah(chip, raw,
	...
	pr_info("cc_uah = %duAh raw->cc = %x cc = %lld after subtracting %x\n",
				*cc_uah, raw->cc,
		...
		int rbatt, int fcc_uah, int uuc_uah, int cc_uah)
{
	...
	/* never adjust during bms reset mode *//* [122112] QC bms reset */
	if (bms_reset) {
					...
					int batt_temp, int chargecycles)
{
	...
	pr_debug("firsttime:%d, soc:%d, shutdown_soc:%d, within_limits:%d\n",
			firsttime, soc, shutdown_soc,
...
battsec:
#if defined(CONFIG_MACH_GODIVA)
		chip->fcc = Samsung_8930_Godiva_2000mAh_data.fcc;
		chip->fcc_temp_lut
			= Samsung_8930_Godiva_2000mAh_data.fcc_temp_lut;
		chip->fcc_sf_lut
			= Samsung_8930_Godiva_2000mAh_data.fcc_sf_lut;
		chip->pc_temp_ocv_lut
			= Samsung_8930_Godiva_2000mAh_data.pc_temp_ocv_lut;
		chip->pc_sf_lut = NULL;
		chip->rbatt_sf_lut
			= Samsung_8930_Godiva_2000mAh_data.rbatt_sf_lut;
		chip->default_rbatt_mohm
			= Samsung_8930_Godiva_2000mAh_data.default_rbatt_mohm;
		chip->delta_rbatt_mohm
			= Samsung_8930_Godiva_2000mAh_data.delta_rbatt_mohm;
#elif defined(CONFIG_MACH_STUNNER)
		chip->fcc = Samsung_8930_Stunner_2000mAh_data.fcc;
		chip->fcc_temp_lut
			= Samsung_8930_Stunner_2000mAh_data.fcc_temp_lut;
		chip->fcc_sf_lut
			= Samsung_8930_Stunner_2000mAh_data.fcc_sf_lut;
		chip->pc_temp_ocv_lut
			= Samsung_8930_Stunner_2000mAh_data.pc_temp_ocv_lut;
		chip->pc_sf_lut = NULL;
		chip->rbatt_sf_lut
			= Samsung_8930_Stunner_2000mAh_data.rbatt_sf_lut;
		chip->default_rbatt_mohm
			= Samsung_8930_Stunner_2000mAh_data.default_rbatt_mohm;
		chip->delta_rbatt_mohm
			= Samsung_8930_Stunner_2000mAh_data.delta_rbatt_mohm;
#else
		chip->fcc = Samsung_8930_Express_2000mAh_data.fcc;
		chip->fcc_temp_lut
			= Samsung_8930_Express_2000mAh_data.fcc_temp_lut;
		chip->fcc_sf_lut
			= Samsung_8930_Express_2000mAh_data.fcc_sf_lut;
		chip->pc_temp_ocv_lut
			= Samsung_8930_Express_2000mAh_data.pc_temp_ocv_lut;
		chip->pc_sf_lut = NULL;
		chip->rbatt_sf_lut
			= Samsung_8930_Express_2000mAh_data.rbatt_sf_lut;
		chip->default_rbatt_mohm
			= Samsung_8930_Express_2000mAh_data.default_rbatt_mohm;
		chip->delta_rbatt_mohm
			= Samsung_8930_Express_2000mAh_data.delta_rbatt_mohm;
#endif
		return 0;
palladium:
...
static int __devinit pm8921_bms_probe(struct platform_device *pdev)
{
	...
	pr_info("%s : r_sense(%d), r_conn(%d)\n", __func__,
		pdata->r_sense_uohm, pdata->rconn_mohm);
