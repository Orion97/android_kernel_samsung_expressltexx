...
static void melfas_ta_cb(struct tsp_callbacks *cb, bool ta_status)
{
	...
	dev_notice(&client->dev, "[TSP] %s\n", __func__);

	info->ta_status = ta_status;

	if (info->enabled) {
		if (info->ta_status) {
			dev_notice(&client->dev, "[TSP] TA connect!!!\n");
			i2c_smbus_write_byte_data(info->client, 0x33, 0x1);
		} else {
			dev_notice(&client->dev, "[TSP] TA disconnect!!!\n");
			i2c_smbus_write_byte_data(info->client, 0x33, 0x2);
			mms_set_noise_mode(info);
		}
	}
...
static int mms100_open_mbinary(struct mms_ts_info *info)
{
	...
	if (machine_is_STUNNER()) {
		ret += request_firmware(&(fw_mbin[0]),\
			"tsp_melfas/BOOT_S.fw", &_client->dev);
		ret += request_firmware(&(fw_mbin[1]),\
			"tsp_melfas/CORE_S.fw", &_client->dev);
		ret += request_firmware(&(fw_mbin[2]),\
			"tsp_melfas/CONF_S.fw", &_client->dev);
	} else if (machine_is_GODIVA()) {
		ret += request_firmware(&(fw_mbin[0]),\
			"tsp_melfas/BOOT_G.fw", &_client->dev);
		ret += request_firmware(&(fw_mbin[1]),\
			"tsp_melfas/CORE_G.fw", &_client->dev);
		ret += request_firmware(&(fw_mbin[2]),\
			"tsp_melfas/CONF_G.fw", &_client->dev);
	}
...
static int mms_ts_fw_info(struct mms_ts_info *info)
{
	...
	info->fw_ic_ver = ver;
	dev_info(&client->dev, "[TSP]fw version 0x%02x !!!!\n", ver);
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (machine_is_STUNNER())
		...
		ret = get_raw_data_tkey(info, 3, VSC_INTENSITY_TK);
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (info->fw_ic_ver == FW_VERSION)
		dev_err(&client->dev, "fw update does not need\n");
	else
		ret = mms_ts_fw_info(info);
#endif
	info->palm_status = 0;
	...
	printk(KERN_INFO "[HASH] Boot:0x%02x, CORE:0x%02x, CONFIG:0x%02x, %d\n",
